...
feature <- function(imgLR, imgHR, n_points=1000){
  
  get_feature <- function(pad_col_idx, pad_row_idx, padd_LRimage = imgLR_temp){
  ...
  for(i in 1:n_files){
    ...
    imgLR[[i]] <- abind(array(0, replace(dim(imgLR[[i]]), 2, 1)),imgLR[[i]], array(0, replace(dim(imgLR[[i]]), 2, 1)), along = 2)
    imgLR_padded <- abind(array(0, replace(dim(imgLR[[i]]), 1, 1)),imgLR[[i]], array(0, replace(dim(imgLR[[i]]), 1, 1)), along = 1)
    imgLR_temp <-imgLR_padded
    imgHR_temp <-imgHR[[i]]
